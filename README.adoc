...
For authenticating to an AWS ECR registry, use link:https://github.com/cloudbees-io/configure-ecr-credentials[the ECR credentials configuration action]:
...
If your source and destination registries are different, you must authenticate to both registries. Invoke the Docker registry login action separately for those registries before invoking the crane action, as in the example below:
